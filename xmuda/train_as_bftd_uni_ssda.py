...
def train(cfg, output_dir='', run_name=''):
    ...
    for iteration in range(start_iteration, max_iteration):
        # fetch data_batches for source & labeled target & unlabeled target
        ...
        # ---------------------------------------------------------------------------- #
        img_s = data_batch_collate['img']
        img_t = data_batch_trg_unlabeled['img']
        ...
        for i in range(img_s.shape[0]):
            n_src = data_batch_collate['img_indices'][i].shape[0]  # num of points in i-th src
            current_number_s = last_number_s + n_src
            xyz_s = data_batch_collate['x'][0][last_number_s:current_number_s, :3]  # coords in i-th src
            ...
            n_trg = data_batch_trg_unlabeled['img_indices'][i].shape[0]  # num of points in i-th trg
            current_number_t = last_number_t + n_trg
            xyz_t = data_batch_trg_unlabeled['x'][0][last_number_t:current_number_t, :3]  # coords in i-th trg
            ...
            index_s = data_batch_collate['x'][0][last_number_s:current_number_s, 3].view(-1, 1)
        ...
        pc_st = [pc_st, data_batch_collate['x'][1]]
        data_batch_st = {
            'x': pc_st,
            'seg_label':  data_batch_collate['seg_label'],
            'img': img_st,
            'img_indices': data_batch_collate['img_indices']
        }
        ...
        feats_fus_st = model_fusion_ema(feats_2d_st.detach(), feats_3d_st.detach(), data_batch_st['img_indices'])
        ...
        # Train on source
        # ---------------------------------------------------------------------------- #
        data_batch_collate['img'] = img_s * lambda_mix + img_t * (1 - lambda_mix)  # mixup

        feats_2d_s = model_2d(data_batch_collate)
        feats_3d_s, preds_3d_s = model_3d(data_batch_collate)

        feats_fus_s = model_fusion(feats_2d_s.detach(), feats_3d_s.detach(), data_batch_collate['img_indices'])

        preds_2d_s = model_2d(data_batch_collate, feats_2d_s, feats_fus_s)

        # segmentation loss: cross entropy
        if class_weights_trg is None:
            seg_loss_src_2d = F.cross_entropy(preds_2d_s['seg_logit'], data_batch_collate['seg_label'],
                                              weight=class_weights_src)
            seg_loss_src_3d = F.cross_entropy(preds_3d_s['seg_logit'], data_batch_collate['seg_label'],
                                              weight=class_weights_src)
        else:
            split_length = len(data_batch_src['x'][0])
            seg_loss_src_2d = F.cross_entropy(preds_2d_s['seg_logit'][:split_length], data_batch_collate['seg_label'][:split_length],
                                              weight=class_weights_src)
            seg_loss_trg_labeled_2d = F.cross_entropy(preds_2d_s['seg_logit'][split_length:], data_batch_collate['seg_label'][split_length:],
                                                      weight=class_weights_trg)
            seg_loss_src_2d = 0.5 * (seg_loss_src_2d + seg_loss_trg_labeled_2d)
            seg_loss_src_3d = F.cross_entropy(preds_3d_s['seg_logit'][:split_length], data_batch_collate['seg_label'][:split_length],
                                              weight=class_weights_src)
            seg_loss_trg_labeled_3d = F.cross_entropy(preds_3d_s['seg_logit'][split_length:], data_batch_collate['seg_label'][split_length:],
                                                      weight=class_weights_trg)
            seg_loss_src_3d = 0.5 * (seg_loss_src_3d + seg_loss_trg_labeled_3d)
        ...
        if cfg.TRAIN.XMUDA.lambda_xm_src > 0:
            seg_logit_2d = preds_2d_s['seg_logit2'] if cfg.MODEL_2D.DUAL_HEAD else preds_2d_s['seg_logit']
            seg_logit_3d = preds_3d_s['seg_logit2'] if cfg.MODEL_3D.DUAL_HEAD else preds_3d_s['seg_logit']
        ...
        with torch.no_grad():
            train_metric_2d.update_dict(preds_2d_s, data_batch_collate)
            train_metric_3d.update_dict(preds_3d_s, data_batch_collate)
            train_metric_fus.update_dict(preds_fus_s, data_batch_collate)
        ...
        # Train on target
        # ---------------------------------------------------------------------------- #
        feats_2d_t = model_2d(data_batch_trg_unlabeled)
        feats_3d_t, preds_3d_t = model_3d(data_batch_trg_unlabeled)

        feats_fus_t = model_fusion(feats_2d_t.detach(), feats_3d_t.detach(), data_batch_trg_unlabeled['img_indices'])

        preds_2d_t = model_2d(data_batch_trg_unlabeled, feats_2d_t, feats_fus_t)
        ...
        if cfg.TRAIN.XMUDA.lambda_pl > 0:
            ...
            # xDPL loss with pseudo labels
            exp_variance_2d = torch.exp(-xm_loss_trg_2d)
            exp_variance_3d = torch.exp(-xm_loss_trg_3d)
            pl_loss_trg_2d = F.cross_entropy(preds_2d_t['seg_logit'], data_batch_trg_unlabeled['pseudo_label_2d'])
            pl_loss_trg_3d = F.cross_entropy(preds_3d_t['seg_logit'], data_batch_trg_unlabeled['pseudo_label_3d'])
            pl_loss_trg_2d = exp_variance_2d * pl_loss_trg_2d
            pl_loss_trg_3d = exp_variance_3d * pl_loss_trg_3d
            train_metric_logger.update(pl_loss_trg_2d=pl_loss_trg_2d, exp_var_2d=exp_variance_2d,
                                       pl_loss_trg_3d=pl_loss_trg_3d, exp_var_3d=exp_variance_3d)
            loss_2d_t.append(cfg.TRAIN.XMUDA.lambda_pl * pl_loss_trg_2d)
            loss_3d_t.append(cfg.TRAIN.XMUDA.lambda_pl * pl_loss_trg_3d)
